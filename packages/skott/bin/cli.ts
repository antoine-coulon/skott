...
cli
    ...
    .option('-c, --exitCodeOnCircularDependencies', 'Specify the exit code to use when circular dependencies are found', commanderParseInt, 1)
    .option('-d, --circularMaxDepth', 'Define the max depth of the nested circular dependencies search', commanderParseInt, Number.POSITIVE_INFINITY)
    .option('-e, --fileExtensions', 'File extensions to explore when building the graph', [...kExpectedModuleExtensions].join(","))
    .option('-m, --displayMode', 'Either display the result of the analysis as a graph, as a file-tree or raw', 'webapp')
    ...
    .option('-ig, --ignorePattern', 'Provide ignore pattern to exclude files from the analysis', '')
    .option('-s, --showCircularDependencies', 'Show all circular dependencies in the graph', false)
    .option('-ts, --tsconfig', 'Provide a path to a tsconfig.json file to use for path aliases resolution', 'tsconfig.json')
    .option('-mf, --manifest', 'Provide a path to a manifest file used for dependencies resolution', 'package.json')
    ...
    .option('-w, --cwd', 'Define the base working directory to use for the analysis. Defaults to the current working directory.', process.cwd())
